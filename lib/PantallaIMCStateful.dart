...
class _PantallaIMCState extends State<PantallaIMCStateful> {
  ...
  Widget build(BuildContext context) {
    ...
    height: 200,
    width: 200,
    child: SvgPicture.asset("assets/images/stethoscope.svg", fit: BoxFit.contain,),
    ),
    SizedBox(
    height: 8,
    ),
    ...
    setState(() {
      ...
      if(_imc >= 18.5 && _imc <= 25) {
        _IMCModelo = IMCModelo(imc: _imc, esNormal: true, resultado: "Tu peso es normal");
      } else if (_imc <18.5) {
        _IMCModelo = IMCModelo(imc: _imc, esNormal: false, resultado: "Tu peso es bajo");
      } else if (_imc > 25) {
        _IMCModelo = IMCModelo(imc: _imc, esNormal: false, resultado: "Tu peso es alto");
      }
    });
